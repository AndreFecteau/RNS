...
Matrix_type create_mid_band_matrix(const solution_vector_type& solution_vector_m,
  ...
  b << 1,0,0,0,(dt*(-4*dx*(-3 + gamma)*Power(rho,3)*u*(um - up) +
    ...
    (12.*Power(dx,2)*Power(rho,3)),(12*Power(dx,2)*Power(rho,3) - 3*dt*Pr*Power(rhom - rhop,2) + 6*dt*Pr*rho*(rhom + rhop) +
      4*dt*dx*(-3 + gamma)*Power(rho,3)*(um - up))/(12.*Power(dx,2)*Power(rho,3)),0,0,
   (dt*((4*(em*gamma*rho*u - ep*gamma*rho*u - e*gamma*rhom*u + e*gamma*rhop*u + e*gamma*rho*um + 3*Power(rho,2)*Power(u,2)*um -
        ...
        (8*exp((2*rho*theta)/((-1 + gamma)*(-2*e + rho*Power(u,2))))*lambda*Q*
         ...
         ((-1 + gamma)*Power(-2*e + rho*Power(u,2),2))))/12.,
   (dt*((4*rho*(-(em*gamma*rho) + ep*gamma*rho + e*gamma*rhom - e*gamma*rhop - 3*Power(rho,2)*u*um + 3*gamma*Power(rho,2)*u*um -
        ...
        ))/(12.*Power(rho,3)),1 + (dt*(-((gamma*(Power(rhom - rhop,2) - 2*rho*(rhom + rhop) + dx*Power(rho,3)*(um - up)))/Power(dx,2)) -
         (8*exp((2*rho*theta)/((-1 + gamma)*(-2*e + rho*Power(u,2))))*lambda*Q*Power(rho,5)*theta*Y)/((-1 + gamma)*Power(-2*e + rho*Power(u,2),2))))
      /(3.*Power(rho,3)),(-2*dt*exp((2*rho*theta)/((-1 + gamma)*(-2*e + rho*Power(u,2))))*lambda*Q*rho)/3.,
   (2*dt*exp((2*rho*theta)/((-1 + gamma)*(-2*e + rho*Power(u,2))))*lambda*
      (4*e*(e*(-1 + gamma) - rho*theta) + 4*rho*(e - e*gamma + rho*theta)*Power(u,2) + (-1 + gamma)*Power(rho,2)*Power(u,4))*Y)/
    (3.*(-1 + gamma)*Power(-2*e + rho*Power(u,2),2)),(dt*((-8*exp((2*rho*theta)/((-1 + gamma)*(-2*e + rho*Power(u,2))))*lambda*Power(rho,2)*theta*u*
           Y)/((-1 + gamma)*Power(-2*e + rho*Power(u,2),2)) + (-Ym + Yp)/dx))/3.,
   (8*dt*exp((2*rho*theta)/((-1 + gamma)*(-2*e + rho*Power(u,2))))*lambda*Power(rho,2)*theta*Y)/(3.*(-1 + gamma)*Power(-2*e + rho*Power(u,2),2)),
...
Matrix_type create_top_band_matrix(const solution_vector_type solution_vector_m,
  ...
  c <<0,dt/(3.*dx),0,0,(dt*(2*dx*(-3 + gamma)*Power(rho,2)*Power(u,2) + 3*Pr*((rhom - rhop)*u + rho*(2*u - um + up))))/(12.*Power(dx,2)*Power(rho,2)),
   -(dt*(3*Pr*(2*rho + rhom - rhop) + 4*dx*(-3 + gamma)*Power(rho,2)*u))/(12.*Power(dx,2)*Power(rho,2)),(dt*(-1 + gamma))/(3.*dx),0,
   (dt*(8*e*gamma*(rho + rhom - rhop) - 4*dx*e*gamma*Power(rho,2)*u +
        rho*(-4*em*gamma + 4*ep*gamma + u*(3*Pr*(rhom - rhop)*u - 4*dx*Power(rho,2)*Power(u,2) + 6*Pr*rho*(u - um + up) +
              4*gamma*((-rhom + rhop)*u + dx*Power(rho,2)*Power(u,2) - 2*rho*(u - um + up))))))/(12.*Power(dx,2)*Power(rho,3)),
   (dt*(2*dx*rho*(2*e*gamma - 3*(-1 + gamma)*rho*Power(u,2)) + (4*gamma - 3*Pr)*((rhom - rhop)*u + rho*(2*u - um + up))))/
    (12.*Power(dx,2)*Power(rho,2)),(dt*gamma*(-rhom + rhop + rho*(-2 + dx*rho*u)))/(3.*Power(dx,2)*Power(rho,2)),0,0,(dt*Y)/(3.*dx),0,
   (dt*(-2 + dx*Le*rho*u))/(3.*Power(dx,2)*Le);
...
Matrix_type create_bot_band_matrix(const solution_vector_type solution_vector_m,
  ...
  Matrix_type a;
    ...
    a << 0,-dt/(3.*dx),0,0,(dt*(-2*dx*(-3 + gamma)*Power(rho,2)*Power(u,2) + 3*Pr*((-rhom + rhop)*u + rho*(2*u + um - up))))/(12.*Power(dx,2)*Power(rho,2)),
   (dt*(-3*Pr*(2*rho - rhom + rhop) + 4*dx*(-3 + gamma)*Power(rho,2)*u))/(12.*Power(dx,2)*Power(rho,2)),-(dt*(-1 + gamma))/(3.*dx),0,
   (dt*(4*e*gamma*(2*(rho - rhom + rhop) + dx*Power(rho,2)*u) +
        rho*(4*em*gamma - 4*ep*gamma + u*(3*Pr*(-rhom + rhop)*u + 4*dx*Power(rho,2)*Power(u,2) -
              4*gamma*((-rhom + rhop)*u + dx*Power(rho,2)*Power(u,2) + 2*rho*(u + um - up)) + 6*Pr*rho*(u + um - up)))))/(12.*Power(dx,2)*Power(rho,3)),
   (dt*(2*dx*rho*(-2*e*gamma + 3*(-1 + gamma)*rho*Power(u,2)) + (4*gamma - 3*Pr)*((-rhom + rhop)*u + rho*(2*u + um - up))))/
    (12.*Power(dx,2)*Power(rho,2)),-(dt*gamma*(-rhom + rhop + rho*(2 + dx*rho*u)))/(3.*Power(dx,2)*Power(rho,2)),0,0,-(dt*Y)/(3.*dx),0,
   -(dt*(2 + dx*Le*rho*u))/(3.*Power(dx,2)*Le);
...
solution_vector_type create_rhs_vector(const solution_vector_type solution_vector_mm,
  ...
  //
  rhs << (2*dt*(0. - (-rhom/(2.*dx) + rhop/(2.*dx))*u - rho*(-um/(2.*dx) + up/(2.*dx))))/3.,
   (2*dt*(0. - (-rhom/(2.*dx) + rhop/(2.*dx))*Power(u,2) + (3*Pr*((-2*u)/Power(dx,2) + um/Power(dx,2) + up/Power(dx,2)))/4. -
        2*rho*u*(-um/(2.*dx) + up/(2.*dx)) - (-1 + gamma)*(-em/(2.*dx) + ep/(2.*dx) - ((-rhom/(2.*dx) + rhop/(2.*dx))*Power(u,2))/2. -
           rho*u*(-um/(2.*dx) + up/(2.*dx)))))/3.,(2*dt*((3*Pr*u*((-2*u)/Power(dx,2) + um/Power(dx,2) + up/Power(dx,2)))/4. -
        ...
        (lambda*Q*rho*Y)/exp((rho*theta)/((-1 + gamma)*(e - (rho*Power(u,2))/2.)))))/3.,
   (2*dt*(-((lambda*rho*Y)/exp((rho*theta)/((-1 + gamma)*(e - (rho*Power(u,2))/2.)))) - (-rhom/(2.*dx) + rhop/(2.*dx))*u*Y -
        rho*(-um/(2.*dx) + up/(2.*dx))*Y + ((-2*Y)/Power(dx,2) + Ym/Power(dx,2) + Yp/Power(dx,2))/Le - rho*u*(-Ym/(2.*dx) + Yp/(2.*dx))))/3.;
  //
