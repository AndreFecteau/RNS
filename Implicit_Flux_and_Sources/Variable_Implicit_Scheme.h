...
Matrix_type create_mid_band_matrix(const solution_vector_type& solution_vector_m,
  ...
  b << 1,0,0,0,(dt*Theta*(-4*dx*(-3 + gamma)*Power(rho,3)*u*(um - up) +
...
Matrix_type create_top_band_matrix(const solution_vector_type solution_vector_m,
  ...
  c << 0,(dt*Theta)/(2*dx + 2*dx*zeta),0,0,(dt*Theta*(2*dx*(-3 + gamma)*Power(rho,2)*Power(u,2) + 3*Pr*((rhom - rhop)*u + rho*(2*u - um + up))))/
    (8.*Power(dx,2)*Power(rho,2)*(1 + zeta)),-(dt*Theta*(3*Pr*(2*rho + rhom - rhop) + 4*dx*(-3 + gamma)*Power(rho,2)*u))/
    (8.*Power(dx,2)*Power(rho,2)*(1 + zeta)),(dt*(-1 + gamma)*Theta)/(2.*dx*(1 + zeta)),0,
   (dt*Theta*(8*e*gamma*(rho + rhom - rhop) - 4*dx*e*gamma*Power(rho,2)*u +
...
Matrix_type create_bot_band_matrix(const solution_vector_type solution_vector_m,
  ...
  Matrix_type a;
    ...
    a << 0,-((dt*Theta)/(2*dx + 2*dx*zeta)),0,0,(dt*Theta*(-2*dx*(-3 + gamma)*Power(rho,2)*Power(u,2) + 3*Pr*((-rhom + rhop)*u + rho*(2*u + um - up))))/
    (8.*Power(dx,2)*Power(rho,2)*(1 + zeta)),(dt*Theta*(-3*Pr*(2*rho - rhom + rhop) + 4*dx*(-3 + gamma)*Power(rho,2)*u))/
    (8.*Power(dx,2)*Power(rho,2)*(1 + zeta)),-(dt*(-1 + gamma)*Theta)/(2.*dx*(1 + zeta)),0,
   (dt*Theta*(4*e*gamma*(2*(rho - rhom + rhop) + dx*Power(rho,2)*u) +
