...
solve(int number_of_frames) {
  ...
  int i = 0;
    plot<global_solution_vector_type>(filename + std::to_string(static_cast<int>(2000)), global_solution_vector, grid.dx());
  ...
  // while (residual > target_residual){
    ...
    residual = time_stepping.timemarch(frame_time_temp, grid.global_solution_vector, flow.lambda);
    int position = 0;
    auto var_vec = Variable_Vector_Isolator<solution_vector_type>(grid.global_solution_vector[position], flow.gamma);
    while (var_vec.rho() > 0.5) {
    ++position;
    var_vec = Variable_Vector_Isolator<solution_vector_type>(grid.global_solution_vector[position], flow.gamma);
    }
    if(isnan(residual) || residual > 1e10){
      ...
      plot<global_solution_vector_type>(filename + std::to_string(static_cast<int>(current_frame)+1), grid.global_solution_vector, time_stepping.get_dx());
