...
#pragma omp parallel for
  for (int i = 0; i < number_of_cells; ++i) {
    if (i < number_of_cells * space_in_front*0.86/domaine_length) {
      ...
      1.0 * 1.0;
    } else if (i < number_of_cells * space_in_front/domaine_length && i > number_of_cells *
              (space_in_front*0.85)/domaine_length) {
      initial_solution[i] << make_RK4_solution_vector(initial_low_mach,
                                                      0.0 * dx + 0.5*dx, gamma, mf);
    } else if (i > number_of_cells * (domaine_length - space_in_back)/domaine_length - 1) {
      initial_solution[i] << make_RK4_solution_vector(initial_low_mach,
                                                      initial_low_mach.length(), gamma, mf);
    } else {
      initial_solution[i] << make_RK4_solution_vector(initial_low_mach,
                                                      (i - space_in_front/domaine_length *
                                                      number_of_cells) * dx + 0.5*dx, gamma, mf);
    }
  }
}
...
double dx = x_max / number_of_cells;
  for (int i = 0; i < number_of_cells; ++i) {
    double x = (i+0.5)*dx;
    initial_solution[i] << sin(x)+10, (sin(x)+10)*cos(x), 1.0/10.0*cos(x) + 1000, (sin(x)+10)*(1.0/3.0*sin(x)+1);
  }
}
